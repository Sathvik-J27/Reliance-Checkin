...
export function RevisitLookup({ onCustomerFound, onBack }: RevisitLookupProps) {
  ...
  const handleFind = async () => {
    ...
    try {
      ...
      const queryParam = searchTerm.includes('@')
        ? searchTerm.toLowerCase()
        : searchTerm.replace(/\D/g, '');

      if (!searchTerm.includes('@') && queryParam.length < 10) {
        setError('Please enter a valid 10-digit phone number');
        setLoading(false);
        return;
      }

      const response = await fetch(`/api/check-ins/lookup?q=${encodeURIComponent(queryParam)}`);
